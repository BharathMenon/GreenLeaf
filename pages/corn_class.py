...
def main():
    st.title(":green[Corn Disease Detection]:corn:")


    uploaded_file = st.file_uploader("Choose an image...", type="jpg")
    st.write("Diseases corn plants may suffer from are:\n Blight, Common Rust and Gray Leaf Spot.")
    if uploaded_file is not None:
        st.image(uploaded_file, caption="Uploaded Image.", use_column_width=True)

        if st.button("Run Model"):

            trigger_url = "https://us-central1-shaped-infusion-403004.cloudfunctions.net/predict"
